% Chapter 6_Question_18

clearvars
clc

rho=0.00; % density 7800 kg/m^3
G = 0.8e11; % in N/m^2

d = [0.01 0.03];   % diameters of shafts in m
    ...
    connect = [1 1 2;   %% First Column is element number
     ...
     coord = [1 0.0;       % first column node second column x coodinate
              2 0.6;
              3 1.0];
     ...
     Ip = [0.01 0.0000001 0.0000001]; % in kg-m^2
...
for i = 1:nele              %loop for finding elemental mass &
    ...
    if(i<nele)
        ...
        mele(:,:,i) = [Ip(nd2) 0;0 Ip(nd2)]+ (rho*J*l/6)*[2 1; 1 2];   %disc is considered in both side
    end
    %assembly
...
disp('Solution is printed to a text file "Chapter 6_Question_18_output_FEM.txt"');
disp('mode shape is saved as "Mode_shape_Chapter 6_Question_18_FEM.fig"');
fid = fopen('Output_Example7_6_FEM.txt','w');
...
ylim([-1.2 1.2]);
legend('show');
saveas(h,'Chapter 6_Question_18_mode_shape_FEM','png');